...
type
  ...
  TFOdtUrbanaRural = class(TForm)
    ...
    qODTHijaDESCRIPCIONTIPOPREDIO: TStringField;
    BitBtn1: TBitBtn;
    qODTApoyo: TQuery;
    ...
    procedure RxDBGrid17Exit(Sender: TObject);
  ...
  // nuevo infraestructura   --Paola
    ...
    procedure BuscarYRenombrarSDF(sDirectorio: string);
  public
    { Public declarations }
...
procedure TFOdtUrbanaRural.sbAgregarRequisicionClick(Sender: TObject);
  var
  QCONSULTA: TQUERY;
  qConsulta2, qStock, qStock2: TQUERY;
  devolucion: string;
begin
  if MessageDlg('Para Agregar una nueva requisición se almacenará la información actual de la ODT hija, desea realizar la operación?',mtConfirmation,[mbYes,mbNo],0) = mrYes then
  Begin
    ...
    begin
     { if qODTPadreCODIGOZONAORIGEN.AsString = '' then
      Begin
        raise Exception.Create('Se necesita la zona de origen, Vaya a la odtpadre y agreguela.');
      end;    }
      if qODTHijaCODIGOGRUPOTRABAJO.AsString = '' then
      Begin
        raise Exception.Create('Se necesita El grupo de Trabajo, por favor Agreguelo par continuar.');
      end;
      fRequisicionMateriales := TfRequisicionMateriales.Create(Application);
    ...
    fRequisicionMateriales.qCOM_M_REQUISICION_MOVI.Open;
      ...
      fRequisicionMateriales.ShowModal;
      ...
      while not qpresmate.Eof do
      begin
        devolucion:= 'SELECT SUM(NVL(r.cantidad_entregada,0)), sum(nvl(r.cantidad_reservada,0)) FROM '+EsquemaAlmacen+'com_m_reserva_inventario'+DBLink+' r, '+EsquemaAlmacen+'com_m_requisicion_movi'+DBLink+' m, '+EsquemaAlmacen+'com_m_requisicion'+DBLink+' c, odtrequisicion i';
        devolucion:= devolucion + ' WHERE i.codigorequisicion = c.cod_requisicion';
        devolucion:= devolucion +' AND i.vigencia=c.vigencia';
        devolucion:= devolucion +' AND i.EMPRESA = c.cias_cias';
        devolucion:= devolucion +' AND c.cod_requisicion=M.cod_requisicion';
        devolucion:= devolucion +' AND c.vigencia = m.vigencia';
        devolucion:= devolucion +' AND c.cias_cias = m.cias_cias';
        devolucion:= devolucion +' AND m.cod_requisicion =r.cod_requisicion';
        devolucion:= devolucion +' AND m.vigencia = r.Vigencia';
        devolucion:= devolucion +' AND m.cias_cias = r.Cias_cias';
        devolucion:= devolucion +' AND m.renglon = r.renglon';
        devolucion:= devolucion +' AND r.cod_catalogo =' + qPresMateCODIGOMATERIAL.AsString;
        devolucion:= devolucion +' AND i.codigoodt = ' +  qODTHijaCODIGOODT.AsString;
        devolucion:= devolucion +' AND i.consecutivo = ' +  qODTHijaCONSECUTIVO.asstring;
        devolucion:= devolucion +' AND c.aprobadasn = ''SI''' ;
        devolucion:= devolucion +' AND C.ESTADO  = ''VIGENTE''';
        qConsulta := TQuery.Create(Application);
        qConsulta.DatabaseName := 'BaseDato';
        qConsulta.SQL.Clear;
        qConsulta.SQL.Add(Devolucion);
        qConsulta.Open;
        qPresMate.edit;
        if  QCONSULTA.Fields[0].ASFLOAT <> 0 then
          dbgMateriales.Columns[5].Field.AsFloat:= QCONSULTA.Fields[0].ASFLOAT;
        if  QCONSULTA.Fields[1].ASFLOAT <> 0 then
          dbgMateriales.Columns[4].Field.AsFloat:= QCONSULTA.Fields[1].ASFLOAT;
        qConsulta.Close;
        qpresmate.next;
      end;
    qConsulta.free;
...
Begin
  ...
  begin
    ...
    fRequisicionMateriales.dbmMotivo.Text:= dbMotivos.Text;
  ...
  qConsulta.free;
  Except
  Application.MessageBox('Error en la transacción, Intentelo de nuevo y si el error persiste consulte con el administrador de Energis.', 'Advertencia', MB_OK + MB_ICONWARNING);
  fRequisicionMateriales.Free;
  end;
  end;
  InsertandoRequisicion:= true;
...
begin
  ...
  try
   ...
   // end;
    ...
    while not qpresmate.Eof do
    begin
      devolucion:= 'SELECT  SUM(nvl(r.cantidad_entregada,0)),sum(nvl(r.cantidad_reservada,0)) FROM '+EsquemaAlmacen+'com_m_reserva_inventario'+DBLink+' r, ';
      devolucion:= devolucion +  EsquemaAlmacen+'com_m_requisicion_movi'+DBLink+' m, '+EsquemaAlmacen+'com_m_requisicion'+DBLink+' c, odtrequisicion i';
      devolucion:= devolucion + ' WHERE i.codigorequisicion = c.cod_requisicion';
      devolucion:= devolucion +' AND i.vigencia=c.vigencia';
      devolucion:= devolucion +' and i.EMPRESA = c.cias_cias';
      devolucion:= devolucion +' AND c.cod_requisicion=M.cod_requisicion';
      ...
      dbgMateriales.Columns[5].Field.AsFloat:= QCONSULTA.Fields[0].ASFLOAT;
      dbgMateriales.Columns[4].Field.AsFloat:= QCONSULTA.Fields[1].ASFLOAT;
      //qPresMateCANTIDADRESERVADA.asfloat:= QCONSULTA.Fields[1].ASFLOAT;
      qConsulta.Close;
      qpresmate.next;
    end;
      qConsulta.free;
...
try
  ...
  qConsulta.ParamByName('CODIGOEMPLEADO').AsFloat := fRequisicionMateriales.qCOM_M_REQUISICIONSOLICITA_AUXI.ASFLOAT;
  qConsulta.Open;
  fRequisicionMateriales.qCOM_M_REQUISICION.fieldbyname('CODIGOSOLICITANTE').AsFloat := qConsulta.Fields[0].AsFloat;
  qConsulta.Close;
  qConsulta.Free;


  //Averiguar LA CEDULA del RESPONSABLE de la requisicion.
  if qODTPadreCODIGOODTTIPOMANOOBRA.AsFloat= 1 then
  begin
  qConsulta := TQuery.Create(Application);
  qConsulta.DatabaseName := 'BaseDato';
  ...
  qConsulta.ParamByName('CODIGOEMPLEADO').AsFloat := fRequisicionMateriales.qCOM_M_REQUISICIONRESPONSABLE_AUXI.AsFloat;
  qConsulta.Open;
  fRequisicionMateriales.qCOM_M_REQUISICION.fieldbyname('CODIGORESPONSABLE').AsFloat := qConsulta.Fields[0].AsFloat;
  qConsulta.Close;
  qConsulta.Free;
  end;
...
begin
  ...
  qConsulta2 := TQuery.Create(Application);
  qConsulta2.DatabaseName := 'BaseDato';
  qConsulta2.sql.text:= 'select impresorequisicion from odtrequisicion where codigorequisicion = '+qRequisicionesCODIGOREQUISICION.asstring +' and '+
                        'codigoodt = '+qRequisicionesCODIGOODT.asstring+ ' and consecutivo = '+qRequisicionesCONSECUTIVO.asstring;
...
//Interfaz OW
{
 ...
 qConsulta2.open;
 if qconsulta2.FieldByName('impresorequisicion').asstring= 'N' then
 begin
   qconsulta2.free;
   raise Exception.Create('Para imprimir esta requisición es necesario guardar la informacion de la ODT Hija primero.');
 end;
 qconsulta2.free;
  ...
  fQRRequisicionMateriales.qCOM_M_RESERVA_INVENTARIO.ParamByName('COD_REQUISICION').AsFloat := codigo_requisicion;
  fQRRequisicionMateriales.qCOM_M_RESERVA_INVENTARIO.Open;
  fQRRequisicionMateriales.QRLabel34.Caption:= '#  '+  inttostr(codigo_requisicion);
  IF qODTPadreNUMEROCONTRATO.ASSTRING = '' THEN
  Begin
    fQRRequisicionMateriales.qrcontrato.Caption:= '';
    fQRRequisicionMateriales.qrNitContratista.caption:= '';
  ...
  BEGIN
     qConsulta2.SQL.Add('SELECT A.CODIGOACTIVO, S.NOMBRESUBESTACION FROM ACTIVOSUBESTACION A, SUBESTACION S');
     qConsulta2.SQL.Add('WHERE A.ACTIVO = ''S'' AND A.CODIGOSUBESTACION = S.CODIGOSUBESTACION AND A.CODIGOSUBESTACION = ''' + qodthijaCODIGOSUBESTACION.AsString + '''' );
  END;
  if fOdturbanarural.qodthijaCODIGOTIPOACTIVO.ASINTEGER = 2 THEN
  BEGIN
     qConsulta2.SQL.Add('SELECT A.CODIGOACTIVO, L.nombreLINEA FROM ACTIVOLINEA A, LINEA L');
     qConsulta2.SQL.Add('WHERE A.ACTIVO = ''S'' AND A.CODIGOLINEA = L.CODIGOLINEA AND A.CODIGOLINEA = ''' + qodthijaCODIGOLINEA.AsString + '''' );
  END;
  if fOdturbanarural.qodthijaCODIGOTIPOACTIVO.ASINTEGER = 3 THEN
  BEGIN
     qConsulta2.SQL.Add('SELECT A.CODIGOACTIVO, C.nombrecircuito FROM ACTIVOCIRCUITO A, circuito C');
     qConsulta2.SQL.Add('WHERE A.ACTIVO = ''S'' AND A.CODIGOCIRCUITO = C.CODIGOCIRCUITO AND A.CODIGOCIRCUITO = ''' + qodthijaCODIGOCIRCUITO.AsString + '''' );
  END;
  qConsulta2.Open;
  if (qConsulta2.Fields[0].AsString <> '') or (qConsulta2.RecordCount = 0) then
    fQRRequisicionMateriales.qrCodigoActivo.Caption:= qConsulta2.Fields[0].AsString
  ...
  fQRRequisicionMateriales.qrRequisicionMateriales.Preview;
  if qODTPadreCODIGOODTTIPOMANOOBRA.ASfloat = 1 then
  begin
  fQRRequisicionMateriales.qrNitContratista.Visible:= false;
  fQRRequisicionMateriales.qrContrato.visible:= false;
...
begin
  ...
  begin
    ...
    qConsulta.open;
    if qconsulta.fieldbyname('suma').asinteger > 0 then Cantidadentregada:= true;
    qConsulta.close;
    qConsulta.SQL.Clear;
    qConsulta.SQL.Add('select proceso from '+EsquemaAlmacen+'com_m_requisicion'+DBLink+' where cod_requisicion = ' + qRequisicionesCODIGOREQUISICION.AsString + ' and vigencia = '+ qRequisicionesVIGENCIA.asstring + ' and cias_cias = '+qRequisicionesEMPRESA.asstring);
    qConsulta.open;
...
//Interfaz OW
{
  ...
  begin
    ...
    qConsulta.close;
    qConsulta.SQL.Clear;
    qConsulta.SQL.Add('select ESTADO from '+EsquemaAlmacen+'inv_m_docu_inven'+DBLink+' where cod_requisicion = ' + qRequisicionesCODIGOREQUISICION.AsString + ' and vigencia_requis = '+ qRequisicionesVIGENCIA.asstring + ' and cias_cias = '+qRequisicionesEMPRESA.asstring);
    qConsulta.open;
    IF qconsulta.fieldbyname('ESTADO').asstring = '' THEN
      ValeSalida:= ''
    ELSE
      ValeSalida:= qconsulta.fieldbyname('ESTADO').asstring;
    qConsulta.Free;

    If (Cantidadentregada = true) or (Proceso = 'FINALIZADA') or (valesalida = 'VIGENTE') then
    Begin
      raise Exception.Create('Esta Requisición no se puede anular pues ya hubo entrega de material o Vale de Salida asociados a ésta.')
    end;
...
fTraslado.qTrasladoMaterialCONSECUTIVO.asinteger:= qODTHijaCONSECUTIVO.asinteger;
fTraslado.qTrasladoMaterialCODIGOTRASLADO.asinteger:=fTraslado.codigotraslado;
//fTraslado.qTrasladoMaterialFECHATRASLADO.asdatetime := dtpFechaTraslado.DateTime;
...
var
  qconsulta, qconsulta2, qTrasladoinicial:tquery;
  nueva,desmantelada:double;
begin
  fTraslado := tfTraslado.Create(Application);
  ...
  qTrasladoinicial := TQuery.Create(Application);
  qTrasladoinicial.DatabaseName := 'BaseDato';
  qTrasladoinicial.SQL.Add('Select codigomaterial, cantidad, cantidaddesmantelada from trasladomaterialdetalle where codigotraslado = '+qTrasladosCODIGOTRASLADO.Asstring);
  qTrasladoinicial.SQL.Add(' and codigoodt = '+qTrasladosCODIGOODT.Asstring+' and consecutivo = '+ qTrasladosCOnsecutivo.Asstring);
  qTrasladoinicial.Open;
  ...
  qpresmate.open;
  fTraslado.free;
  qTrasladoinicial.free;

{fTraslado := tfTraslado.Create(Application);
ftraslado.Estado:= 'Edit';
ftraslado.DBNavigator2.VisibleButtons:= [nbPrior,nbNext,nbInsert,nbEdit,nbPost,nbCancel];
ftraslado.qTrasladoMaterial.ParamByName('codigotraslado').asinteger := qTrasladosCODIGOTRASLADO.AsInteger;
ftraslado.qTrasladoMaterial.ParamByName('codigoodt').asfloat := qTrasladosCODIGOODT.AsFloat;
ftraslado.qTrasladoMaterial.ParamByName('consecutivo').asINTEGER := qTrasladosCOnsecutivo.AsINTEGER;
...
procedure TFOdtUrbanaRural.RxDBGrid17Exit(Sender: TObject);
var
qconsulta:tquery;
ValorIntervencion: double;
desmantelado, nuevo:double;
begin
//Interfaz OW
{
ValorIntervencion:= 0;
...
begin
  if qPresMateCANTIDADCONSUMIDA.asstring = '' then
    ...
    nuevo:= qPresMateCANTIDADCONSUMIDA.asfloat;

  if qPresMateCANTIDADCONSUMIDADESMANTELADA.asstring = '' then
    desmantelado:= 0
  else
    desmantelado:= qPresMateCANTIDADCONSUMIDADESMANTELADA.asfloat;

  qConsulta := TQuery.Create(Application);
  qConsulta.DatabaseName := 'BaseDato';
  qConsulta.SQL.Add('select CODIGOMATERIAL, VALORUNITARIO from MATERIALESSA');
  qConsulta.SQL.Add('where CODIGOMATERIAL = '+ qPresMateCODIGOMATERIAL.asstring );
  qConsulta.open;
  ValorIntervencion:= ValorIntervencion + ((nuevo + desmantelado)* qconsulta.Fields[1].AsFloat);
  qConsulta.free;
  qpresmate.Next;
end;
...
qConsulta := TQuery.Create(Application);
qConsulta.DatabaseName := 'BaseDato';
qConsulta.SQL.Add('update '+EsquemaAlmacen+'saf_m_intervenciones'+DBLink+' set valor_intervencion = '+floattostr(ValorIntervencion));
qConsulta.SQL.Add('where cod_intervencion= (select codigointervencion from odtintervencion');
qConsulta.SQL.Add('where codigoodt = '+qODTHijaCODIGOODT.asstring);
qConsulta.SQL.Add('and consecutivo = '+qODTHijaCONSECUTIVO.asstring+')');
qConsulta.execsql;
qConsulta.Free; }
{
IF qpresmate.UpdatesPending = TRUE THEN
qpresmate.post;    }
...
var
  ...
  Valstock:double;
begin
  if (qPresMateCANTIDADRETIRADABUENA.asstring = '') then  qPresMateCANTIDADRETIRADABUENA.asfloat:= 0;
  if qPresMateCANTIDADRETIRADABUENA.asfloat < 0 then  raise Exception.Create('La cantidad debe ser positiva.');
  //Interfaz OW
  {qConsulta := TQuery.Create(Application);
  qConsulta.DatabaseName := 'BaseDato';
  qConsulta.SQL.Add('select (NVL(cantidadactualdesmanteladabuen,0)+' + qPresMateCANTIDADRETIRADABUENA.AsString + ' - ' + floattostr(RETIRADABUENA) + ') as stock FROM stockgrupotrabajo');
  qConsulta.SQL.Add('where codigogrupotrabajo= '+ qodthijaCODIGOGRUPOTRABAJO.asstring );
  qConsulta.SQL.Add('and codigoadministrativo= '+qodtpadreCODIGOADMINISTRATIVO.asstring);
  qConsulta.SQL.Add('and codigomaterial = '+qpresmateCODIGOMATERIAL.asstring);
  qConsulta.Open;
  Valstock:= qConsulta.FieldByName('stock').AsFloat;
  qConsulta.free;
  if  Valstock < 0 then
    raise Exception.Create('Con ésta cantidad el stock daría negativo, revise bien la operación.');}
end;
...
Valstock:double;
begin
  if (qPresMateCANTIDADRETIRADAMALA.asstring = '') then  qPresMateCANTIDADRETIRADAMALA.asfloat:= 0;
  if qPresMateCANTIDADRETIRADAMALA.asfloat < 0 then  raise Exception.Create('La cantidad debe ser positiva.');
  //Interfaz OW
  {qConsulta := TQuery.Create(Application);
  qConsulta.DatabaseName := 'BaseDato';
  qConsulta.SQL.Add('select (NVL(cantidadactualdesmanteladamala,0)+' + qpresmateCANTIDADRETIRADAMALA.AsString + ' - ' + floattostr(RETIRADAMALA) + ') as stock FROM stockgrupotrabajo');
  qConsulta.SQL.Add('where codigogrupotrabajo= '+ qodthijaCODIGOGRUPOTRABAJO.asstring );
  qConsulta.SQL.Add('and codigoadministrativo= '+qodtpadreCODIGOADMINISTRATIVO.asstring);
  qConsulta.SQL.Add('and codigomaterial = '+qpresmateCODIGOMATERIAL.asstring);
  qConsulta.Open;
...
begin
//Interfaz OW
  {IF qODTHijaCODIGOTIPOTRABAJOODT.ASFLOAT = 1 THEN
  bEGIN
    qConsulta2 := TQuery.Create(Application);
    qConsulta2.DatabaseName := 'BaseDato';
    if qodthijaCODIGOTIPOACTIVO.ASINTEGER = 1 THEN
    ...
    END;
    qConsulta2.Open;
    IF  (qConsulta2.RecordCount > 0) and (intervencioncreada = false) THEN
    Begin
      qConsulta2.Close;
      qConsulta2.Free;
      raise Exception.Create('Se esta trabajando sobre un activo de la Empresa, por favor genere la intervencion.');
    end;
  END; }
  if (qPresMateCANTIDADCONSUMIDA.asstring = '') then  qPresMateCANTIDADCONSUMIDA.asfloat:= 0;
  if qPresMateCANTIDADCONSUMIDA.asfloat < 0 then  raise Exception.Create('La cantidad debe ser positiva.');
...
begin
  ...
  if (qPresMateCANTIDADCONSUMIDADESMANTELADA.asstring = '') then  qPresMateCANTIDADCONSUMIDADESMANTELADA.AsFloat:= 0;
  if qPresMateCANTIDADCONSUMIDADESMANTELADA.asfloat < 0 then  raise Exception.Create('La cantidad debe ser positiva.');
  //Interfaz OW
  qConsulta := TQuery.Create(Application);
  qConsulta.DatabaseName := 'BaseDato';
    ...
    case sender.asinteger of
      ...
      begin
        ...
        query.Databasename:='BaseDato';
          ...
          begin
            pcMaterial.Pages[i].Enabled:=false;
          end;
...
begin
  ...
  for i := 1 to pcMaterial.PageCount-1 do
  begin
    pcMaterial.Pages[i].Enabled:=true;
  end;
...
begin
  qConsulta := TQuery.Create(Application);
  qConsulta.DatabaseName := 'BaseDato';
  qConsulta.SQL.Add('SELECT nvl(max(codigoreintegro),0) FROM odtreintegro');
  qConsulta.Open;
  ...
  fReintegro.codigoReintegro:= CodigoReintegro +1;
...
begin
  ...
  if Application.MessageBox('Esta seguro de eliminar este registro? ','Advertencia!', mb_yesNoCancel)=mrYes then
  begin
    {if qReintegrosESTADO.asstring = 'P' THEN
      raise Exception.Create('El reintegro ya fue procesado por almacen y no puede ser eliminado.');}
...
var
  qconsulta, qconsulta2, qReintegroinicial:tquery;
  nueva,desmantelada:double;
begin
  if ((qReintegrosESTADO.asstring = 'A') or (qReintegrosESTADO.asstring = 'P')) THEN
  ...
  qReintegroinicial := TQuery.Create(Application);
  qReintegroinicial.DatabaseName := 'BaseDato';
  qReintegroinicial.SQL.Add('Select codigomaterial, CANTIDADNUEVAB, CANTIDADNUEVAM, CANTIDADDESMANB , CANTIDADDESMANM from ODTREINTEGRODETALLE where codigoreintegro = '+qReintegrosCODIGOREINTEGRO.Asstring);
  qReintegroinicial.Open;
  ...
  begin
   { if Application.MessageBox(PChar('Si imprime esta nota se bloqueará y no permitira imprimirla nuevamente. Esta seguro que desea hacerlo?'),PChar('Confirmación'),MB_ICONQUESTION+MB_YESNO)=idNo then
    ...
    try
      ...
      fQRodtDevoluciones.qrlNumeroReintegro.caption:=  'Reintegro #  '+qReintegroSCODIGOREINTEGRO.asString;
      fQRodtDevoluciones.qCia.Open;
      fQRodtDevoluciones.qCliente.open;
      fQRodtDevoluciones.qmaterial.open;
      fQRodtDevoluciones.qunidad.open;
      fQRodtDevoluciones.qEncabezado.Close;
      fQRodtDevoluciones.qEncabezado.ParamByName('codigoreintegro').asfloat:= qReintegroSCODIGOREINTEGRO.asfloat;
      fQRodtDevoluciones.qEncabezado.Open;
      fQRodtDevoluciones.qReintegro.Close;
      fQRodtDevoluciones.qReintegro.ParamByName('codigoreintegro').asfloat:= qReintegroSCODIGOREINTEGRO.asfloat;
...
procedure TFOdtUrbanaRural.BuscarYRenombrarSDF(sDirectorio: string);
var
  Busqueda: TSearchRec;
  iResultado: Integer;
begin
  iResultado :=  FindFirst( sDirectorio + '*.sdf', faAnyFile, Busqueda );
  while iResultado = 0 do
  begin
    RenameFile(sDirectorio+Busqueda.Name,sDirectorio+'EnerGisMóvil.sdf');
