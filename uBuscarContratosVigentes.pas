...
type
  TfBuscarContratosVigentes = class(TForm)
    Panel1: TPanel;
    Panel2: TPanel;
    Panel3: TPanel;
    dbGrid1: TRxDBGrid;
    qListaContratos: TQuery;
...
var
  fBuscarContratosVigentes: TfBuscarContratosVigentes;
implementation

uses uFiltroContrato;

{$R *.DFM}

procedure TfBuscarContratosVigentes.dbGrid1MouseMove(Sender: TObject;
...
procedure TfBuscarContratosVigentes.FormCreate(Sender: TObject);
begin
  ListaOrden:=TStringList.Create;
  FFiltroContrato:=TFFiltroContrato.Create(Application);
  FFiltroContrato.Query := qListaContratos;
end;

procedure TfBuscarContratosVigentes.dbGrid1TitleClick(Column: TColumn);
{$J+}
...
begin
  ...
  begin
    ...
    if (ListaOrden.Count > 0) and (not Encontrado) then
     ...
     qListaContratos.SQl.Delete(qListaContratos.SQl.Count - 1);
     Sort:='Order By ';
     for i:=0 to ListaOrden.Count - 1 do
       begin
          ListaTemp:=TStringList.Create;
     ...
     qListaContratos.Sql.Add(Sort);
     qListaContratos.Close;
     qListaContratos.Open;
  end;

end;

procedure TfBuscarContratosVigentes.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
 ListaOrden.Free;
 FFiltroContrato.Free;
end;

procedure TfBuscarContratosVigentes.SBFiltrarClick(Sender: TObject);
begin
  FFiltroContrato.Show;
end;


procedure TfBuscarContratosVigentes.Enviar(xx,aForm: TForm);
var
  d,l,t: integer;
begin
...
procedure TfBuscarContratosVigentes.AppEvents1Idle(Sender: TObject;
  var Done: Boolean);
begin
  Enviar(self,FFiltroContrato);
end;

procedure TfBuscarContratosVigentes.LimpiarFiltro;
var i,nLineas:Integer;
begin
  nLineas:=qListaContratos.SQL.Count - 4;
  if nLineas > 0 then
    for i:=1 to nLineas do
       qListaContratos.SQL.Delete(3);
end;
