unit ufiltroMantenimientoAnterior;
...
type
  TfFiltroMantenimientoAnterior = class(TForm)
    Panel2: TPanel;
    Panel3: TPanel;
...
var
  fFiltroMantenimientoAnterior: TfFiltroMantenimientoAnterior;

implementation

uses uFormaFecha, uListaRegistrarMantenimientoAnterior;

{$R *.DFM}

function TfFiltroMantenimientoAnterior.GetFechaHora: String;
begin
...
procedure TfFiltroMantenimientoAnterior.BitBtnAnularFiltroClick(
  Sender: TObject);
begin
  qmantenimientos.Close;
end;

procedure TfFiltroMantenimientoAnterior.FormCreate(Sender: TObject);
begin
...
procedure TfFiltroMantenimientoAnterior.bitBFiltrarClick(Sender: TObject);
begin
  if(rbSubestacion.Checked=true)then
  begin
    qMantenimientos.SQL.Clear;
    qMantenimientos.Close;
    qMantenimientos.SQL.Text:=' select distinct t.codigotarea,t.fechainicial,t.fechafinal,t.nombretarea,nt.tension,et.nombreestado,'+
                              ' e.nombretipoelemento,s.nombresubestacion,m.codigomantenimiento,m.codigotipomant,'+
                              ' tm.nombremant,mo.codigoodt,mo.consecutivo,a.nombreareaoperacionodt'+
                              ' from pm_tarea t,pm_mantenimiento m, pm_tipomantenimiento tm,PM_MANTENIMIENTOODT mo,odtdia od,'+
                              ...
                              ' and mo.codigomantenimiento=m.codigomantenimiento'+
                              ' and bm.codigoareaoperacionodt=a.codigoareaoperacionodt'+
                              ' and od.codigoodt=mo.codigoodt'+
                              ' and od.consecutivo=mo.consecutivo'+
                              ' and ((od.odtgenerada is null) or (od.odtgenerada<>''SI'') or (od.odtgenerada<>''si''))';
...
procedure TfFiltroMantenimientoAnterior.DBLCBSubestacionClick(Sender: TObject);
begin
  if DBLCBSubestacion.Text<>'' then
  begin
...
procedure TfFiltroMantenimientoAnterior.LineaClick(Sender: TObject);
...
procedure TfFiltroMantenimientoAnterior.cbElementoChange(Sender: TObject);
var sqltablaelemento:string;
begin
...
procedure TfFiltroMantenimientoAnterior.rbLineaClick(Sender: TObject);
begin
  if qCircuito.Active then
    qCircuito.Close;
  if lLinea.Enabled = false then
...
procedure TfFiltroMantenimientoAnterior.rbSubestacionClick(Sender: TObject);
begin
  if qCircuito.Active then
    qCircuito.Close;
...
procedure TfFiltroMantenimientoAnterior.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
//  Action:=cafree;
end;

procedure TfFiltroMantenimientoAnterior.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if (Key = #13) then
...
procedure TfFiltroMantenimientoAnterior.ElementoClick(Sender: TObject);
var sqltablaelemento:string;
begin
  if rbLinea.Checked then
  begin
