...
var
  FQRMttosFecha: TFQRMttosFecha;

implementation

{$R *.DFM}

procedure TFQRMttosFecha.QuickRep1AfterPreview(Sender: TObject);
begin
  close;
...
procedure TFQRMttosFecha.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:=cafree;
end;

procedure TFQRMttosFecha.qMttoReprogramadoCalcFields(DataSet: TDataSet);
begin
  qElementoMtto.Close;
...
procedure TFQRMttosFecha.consultarElemento(codigotipoelemento:string);
...
procedure TFQRMttosFecha.FormCreate(Sender: TObject);
begin
  qElementoMtto.Open;
end;
