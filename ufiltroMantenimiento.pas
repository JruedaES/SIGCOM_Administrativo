...
type
  TfFiltroMantenimiento = class(TForm)
    Panel2: TPanel;
    Panel3: TPanel;
...
var
  fFiltroMantenimiento: TfFiltroMantenimiento;
...
function TfFiltroMantenimiento.GetFechaHora: String;
begin
...
procedure TfFiltroMantenimiento.FormCreate(Sender: TObject);
begin
...
procedure TfFiltroMantenimiento.bitBFiltrarClick(Sender: TObject);
begin
  ...
  begin
    ...
    qMantenimientos.SQL.Text:=' select distinct t.codigotarea,t.fechainicial,t.fechafinal,t.nombretarea,nt.tension,et.nombreestado,'+
                              ...
                              ' from pm_tarea t,pm_mantenimiento m, pm_tipomantenimiento tm,PM_MANTENIMIENTOODT mo,AREAOPERACIONODT a,'+
...

{
    ...
    qMantenimientos.SQL.Add('order by fechainicial desc');
//  end;
...
procedure TfFiltroMantenimiento.DBLCBSubestacionClick(Sender: TObject);
begin
  if DBLCBSubestacion.Text<>'' then
  begin
...
procedure TfFiltroMantenimiento.LineaClick(Sender: TObject);
...
procedure TfFiltroMantenimiento.rbLineaClick(Sender: TObject);
begin
  if qCircuito.Active then
    qCircuito.Close;
...
procedure TfFiltroMantenimiento.rbSubestacionClick(Sender: TObject);
begin
  if qCircuito.Active then
    qCircuito.Close;
...
procedure TfFiltroMantenimiento.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
//  Action:=cafree;
end;

procedure TfFiltroMantenimiento.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if (Key = #13) then
...
procedure TfFiltroMantenimiento.ElementoClick(Sender: TObject);
var sqltablaelemento:string;
begin
