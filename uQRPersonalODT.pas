...
type
  TFQRPersonalODT = class(TForm)
    ...
    QRLabel2: TQRLabel;
    QRDBText1: TQRDBText;
    qSobreTiempoMOTIVOS: TStringField;
...
var
  FQRPersonalODT: TFQRPersonalODT;

implementation

{$R *.DFM}

procedure TFQRPersonalODT.QRGroup1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Sender.Color := clSilver;
end;

procedure TFQRPersonalODT.FormShow(Sender: TObject);
begin
  QuickRep.Preview;
end;

procedure TFQRPersonalODT.QuickRep1AfterPreview(Sender: TObject);
begin
  Close;
end;

procedure TFQRPersonalODT.FormCreate(Sender: TObject);
begin
  ListaRotulos := TStringList.Create;
  ListaDatos   := TStringList.Create;
end;

procedure TFQRPersonalODT.FormDestroy(Sender: TObject);
begin
  ListaRotulos.Free;
  ListaDatos.Free;
end;

procedure TFQRPersonalODT.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
...
procedure TFQRPersonalODT.FormClose(Sender: TObject;
...
procedure TFQRPersonalODT.QRDBText1Print(sender: TObject;
  var Value: String);
begin
  if length(value)>90 then value:=copy(value,1,90);
end;
