...
type
  TFFormatoMttoDatos = class(TForm)
    Panel1: TPanel;
    Panel2: TPanel;
...
var
  FFormatoMttoDatos:TFFormatoMttoDatos;

implementation

uses uFuncionSGD, uODTUrbanaRural,uHojaVidaEquipo;
{$R *.DFM}
procedure TFFormatoMttoDatos.qFormatoDatosAfterInsert(DataSet: TDataSet);
var qDatos:TQuery;
...
procedure TFFormatoMttoDatos.FormShow(Sender: TObject);
var
...
begin
  ...
  //qrDatos.SQL.Add('SELECT NOMBREAMBITO FROM PM_AMBITO WHERE CODIGOAMBITO = 1' );
  qrDatos.Open;
  ...
  if hojavida then
  begin
//    qrDatos.SQL.Add('SELECT NOMBREFORMATOMTTO FROM HVE_FORMATOMTTO WHERE CODIGOFORMATOMTTO='+ fHojaVidaEquipo.qPruebasProgramadasCODIGOFORMATOMTTO.AsString);
    qrDatos.SQL.Add('SELECT NOMBREFORMATOMTTO FROM HVE_FORMATOMTTO WHERE CODIGOFORMATOMTTO='+ fHojaVidaEquipo.qFormatoDatosCODIGOFORMATOMTTO.AsString);
    qrDatos.Open;
    ENombreFormato.text:=qrDatos.Fields[0].AsString;
    qrDatos.SQL.Clear;
//    ECodigoFormato.text:= fHojaVidaEquipo.qPruebasProgramadasCODIGOFORMATOMTTO.AsString;
    ECodigoFormato.text:= fHojaVidaEquipo.qFormatoDatosCODIGOFORMATOMTTO.AsString;
  end;
...
procedure TFFormatoMttoDatos.qFormatoDatosCalcFields(DataSet: TDataSet);
var qusuario:tquery;
begin
...
procedure TFFormatoMttoDatos.CambiaCodigoFormato;
...
procedure TFFormatoMttoDatos.qEncabezadoDatosAfterPost(DataSet: TDataSet);
begin
  qEncabezadoDatos.ApplyUpdates;
  qEncabezadoDatos.CommitUpdates;
end;

procedure TFFormatoMttoDatos.qEncabezadoDatosAfterDelete(
  DataSet: TDataSet);
begin
  qEncabezadoDatos.ApplyUpdates;
  qEncabezadoDatos.CommitUpdates;
...
procedure TFFormatoMttoDatos.qEncabezadoDatosAfterInsert(
  DataSet: TDataSet);
var query:tquery;
begin
...
procedure TFFormatoMttoDatos.qEncabezadoDatosCalcFields(DataSet: TDataSet);
var query:tquery;
begin
...
procedure TFFormatoMttoDatos.qFormatoDatosCODIGOADMINISTRATIVOValidate(
  Sender: TField);
begin
  if Sender.Text <> '' then
  begin
...
procedure TFFormatoMttoDatos.qFormatoDatosAfterPost(DataSet: TDataSet);
begin
  qFormatoDatos.ApplyUpdates;
  qFormatoDatos.CommitUpdates;
end;

procedure TFFormatoMttoDatos.qFormatoDatosAfterDelete(DataSet: TDataSet);
begin
  qFormatoDatos.ApplyUpdates;
  qFormatoDatos.CommitUpdates;
end;

procedure TFFormatoMttoDatos.bAceptarClick(Sender: TObject);
begin
...
procedure TFFormatoMttoDatos.FormClose(Sender: TObject;
...
procedure TFFormatoMttoDatos.bCancelarClick(Sender: TObject);
begin
//  if updFormatoDatos = nil then
//  begin
    ModalResult:=MrCancel;
...
procedure TFFormatoMttoDatos.qAmbitoAfterDelete(DataSet: TDataSet);
begin
  qAmbito.ApplyUpdates;
  qAmbito.CommitUpdates;
end;

procedure TFFormatoMttoDatos.qAmbitoAfterPost(DataSet: TDataSet);
begin
  qAmbito.ApplyUpdates;
  qAmbito.CommitUpdates;
end;

procedure TFFormatoMttoDatos.cbElementoChange(Sender: TObject);
begin
  qFormatoDatos.FieldByName('CODIGOELEMENTO').AsInteger:= strtoint(eElemento.text);
end;

procedure TFFormatoMttoDatos.qFormatoDatosCODIGOAMBITOValidate(
  Sender: TField);
begin
...
procedure TFFormatoMttoDatos.qFormatoDatosCODIGOCOORDINACIONValidate(
  Sender: TField);
begin
...
procedure TFFormatoMttoDatos.qFormatoDatosCODIGOCIRCUITOValidate(
  Sender: TField);
begin
...
procedure TFFormatoMttoDatos.qFormatoDatosBeforePost(DataSet: TDataSet);
begin
  RefrescarVariables;
...
procedure TFFormatoMttoDatos.qFormatoDatosCODIGOUSUARIOCREACIONValidate(
  Sender: TField);
var qusuario:tquery;
begin
  RefrescarVariables;
...
procedure TFFormatoMttoDatos.qFormatoDatosCODIGOFORMATOMTTOChange(
  Sender: TField);
var qEliminarEncabezados:tquery;
begin
  if qFormatoDatosCODIGOFORMATOMTTO.AsString <> '' then
...
procedure TFFormatoMttoDatos.FormCreate(Sender: TObject);
begin
  hojavida:=false;
end;
